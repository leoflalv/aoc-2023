...
func getPiecesSum(matrix [][]rune) int {
	...
	// Initialize accum and valid arrays
	accum := make([][]int, len(matrix))
	valid := make([][]bool, len(matrix))
	for y := 0; y < len(matrix); y++ {
		accum[y] = make([]int, len(matrix[y]))
		valid[y] = make([]bool, len(matrix[y]))
	}

	for y := 0; y < len(matrix); y++ {
		for x := 0; x < len(matrix[y]); x++ {
			for i := 0; i < 8; i++ {
				if unicode.IsDigit(matrix[y][x]) && checkX(x+xPos[i], matrix) && checkY(y+yPos[i], matrix) && matrix[y+yPos[i]][x+xPos[i]] == '*' {
					var lastIndex int

					// Update accum and valid matrix
					if accum[y+yPos[i]][x+xPos[i]] == 0 {
						num, currIndex := getNumber(matrix, x, y)
						accum[y+yPos[i]][x+xPos[i]] = num
						lastIndex = currIndex
					} else {
						num, currIndex := getNumber(matrix, x, y)
						accum[y+yPos[i]][x+xPos[i]] *= num
						valid[y+yPos[i]][x+xPos[i]] = true
						lastIndex = currIndex
					}
	...
	// Get sum of valid positions
	for y := 0; y < len(valid); y++ {
		for x := 0; x < len(valid[y]); x++ {
			if valid[y][x] {
				sum += accum[y][x]
			}
		}
